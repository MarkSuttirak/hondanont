...
const BlogAdmin = () => {
  const { data, isLoading, error, mutate } = useFrappeGetDocList('Honda Blog Category', {
  ...
  useFrappeDocTypeEventListener('Honda Blog Category', (d) => {
    console.log(d)
      if (d.doctype === 'Honda Blog Category'){
      mutate()
    }
  })

  const updateCate = (info) => {
    updateDoc('Honda Blog Category', data[rowNum].name, info)
    .then(() => {
      closeUpdateCate();
      toast({
  ...
  const deleteCate = (info) => {
    deleteDoc('Honda Blog Category', data[rowNum].name, info)
    .then(() => {
      closeDeleteCate();
      setRowNum(null);
      toast({
